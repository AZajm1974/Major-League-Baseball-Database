setwd("C:/Users/Owner/OneDrive/Documents/Job Search Training/Training/Dataquest/SQL/Intermediate/GP_designing and creating database")
library(RSQLite)
library(DBI)
library(tidyverse)

##Import Data Using "Import Dataset" function in upper right consule.
...
##It looks like the game log has a record of over 170,000 games. 
##It looks like these games are chronologically ordered and occur between 1871 and 2016.
...
##In the game log, each player has a defensive position listed, which seems to be a number between 1-10. 
##Doing some research around this, I found this [article] (http://probaseballinsider.com/baseball-instruction/baseball-basics/baseball-basics-positions/) which gives us a list of names for each numbered position:
...
##This seems to be a list of people with IDs. The IDs look like they match up with those used in the game log. 
##There are debut dates, for players, managers, coaches and umpires. 
##We can see that some people might have been one or more of these roles.
##It also looks like coaches and managers are two different things in baseball. 
##After some research, managers are what would be called a 'coach' or 'head coach' in other sports, and coaches are more specialized, like base coaches. 
##It also seems like coaches aren't recorded in the game log.

park <- read_csv("park_codes.csv")
view(park)

##This seems to be a list of all baseball parks. 
##There are IDs which seem to match with the game log, as well as names, nicknames, city and league.
...
check_query_2 <- "SELECT * FROM person LIMIT 50;"

check_2 <- dbGetQuery(conn, check_query_2)
head(check_2)
...
check_query_3 <- "SELECT * FROM park LIMIT 50;"
...
check_4 <- dbGetQuery(conn, check_query_4)
head(check_4)
...
check_5 <- dbGetQuery(conn, check_query_5)
head(check_5)
...
check_query_6 <- "SELECT * FROM team LIMIT 50;"

check_6 <- dbGetQuery(conn, check_query_6)
head(check_6)
...
check_query_7 <- "SELECT * FROM game LIMIT 50;"

check_7 <- dbGetQuery(conn, check_query_7)
head(check_7)
...
check <- dbGetQuery(conn, check_query_9)
head(check)

##This particular solution incorporates a bit of regular expression and some nested for loops. 
##If you need to review for-loops again, refer to our older mission on [control structures](https://www.dataquest.io/m/338-working-with-control-structures/). 
##Regular expression is a technique where we look for specific patterns in a character vector and replace it with another pattern. 
##In this case, we need to replace all the instances of "%s" and "%f" with a new letter and number, respectively. 
##In particular, the `gsub()` function replaces *all* instances of "%s" and "%f" respectively. 
## With each run of the nested loop, we get a properly formatted query that fulfills our needs. 
##Using loops is one potential solution, but it's also possible to vectorize this work and recreate what we need.
